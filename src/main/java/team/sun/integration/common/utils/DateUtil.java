...
public class DateUtil {
    ...
    /**
     * JDK1.8时间转换字符串 格式：yyyy-MM-dd HH:mm:ss
     **/
    public static String dateToString(Date date) {
        ...
        return date.toInstant().atZone(zoneId).toLocalDateTime().format(format);
    }
    ...
    public static Date stringToDate(String dateString) {
        ...
        ;
        //默认时区为东8区
        return Date.from(LocalDateTime.parse(dateString, format).toInstant(ZoneOffset.of("+8")));
    }
     ...
     *
     * @param objDateTime 以"yyyy-MM-dd HH:mm:ss"为格式的时间字符串
     * @return 转换后的日期
     */
    public static Date parseDateTime(Object objDateTime) {
        Date date = new Date();
        if (objDateTime == null || "".equals(objDateTime.toString().trim())) {
            return date;
        } else {
            String strDateTime = objDateTime.toString().trim();
            try {
    ...
    /**
     * 得到日期所在月份的开始日期（第一天的开始日期），
     * 例如2004-1-15 15:10，转换后为2004-1-1 00:00
     *
     * @param date 需要转换的日期
     * @return 日期所在月份的开始日期 *
     */
    ...
    public static void main(String[] args) {
        String ss = DateUtil.dateToString(DateUtil.stringToDate("2018-10-22 21:22:32"));
