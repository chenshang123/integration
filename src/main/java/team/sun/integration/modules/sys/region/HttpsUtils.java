package team.sun.integration.modules.sys.region;
...
public class HttpsUtils {
    ...
    public static void main(String[] args) {
        JSONObject jsonObject = HttpsUtils.doGetAuthorization("https://apis.map.qq.com/ws/district/v1/list?key=DFEBZ-FC3AU-A24VQ-2ZAAZ-AAGQK-ZHBZJ");
        ...
        for (int i = 0; i < s1.size(); i++) {
            ...
            st = js.get("location").toString().replaceFirst("\\{\"lng\":", "").replaceFirst("\\}", "");
            //System.out.println(js.get("id")+" "+js.get("name")+" "+js.get("pinyin")+" "+js.get("fullname")+" "+st.split(",\"lat\":")[0]+" "+st.split(",\"lat\":")[1]+" "
            //       +id+" "+1+" "+(i+1));
            JSONObject jsonObject1 = JSONObject.parseObject(HttpsUtils.Get("https://apis.map.qq.com/ws/district/v1/getchildren?key=DFEBZ-FC3AU-A24VQ-2ZAAZ-AAGQK-ZHBZJ&id=" + js.get("id")));
            JSONArray array1 = jsonObject1.getJSONArray("result");
            JSONArray array2 = array1.getJSONArray(0);
            for (int j = 0; j < array2.size(); j++) {
                js = array2.getJSONObject(j);
                st = js.get("location").toString().replaceFirst("\\{\"lng\":", "").replaceFirst("\\}", "");
                //System.out.println(js.get("id")+" "+js.get("name")+" "+js.get("pinyin")+" "+js.get("fullname")+" "+st.split(",\"lat\":")[0]+" "+st.split(",\"lat\":")[1]+" "
                //+id+" 100000 "+3+" "+(j+1));

                id = js.get("id").toString();
                JSONObject jsonObject2 = JSONObject.parseObject(HttpsUtils.Get("https://apis.map.qq.com/ws/district/v1/getchildren?key=DFEBZ-FC3AU-A24VQ-2ZAAZ-AAGQK-ZHBZJ&id=" + js.get("id")));
                JSONArray array3 = jsonObject2.getJSONArray("result");
                JSONArray array4 = array3.getJSONArray(0);
                for (int l = 0; l < array4.size(); l++) {
                    js = array4.getJSONObject(l);
                    System.out.println(js.get("id") + " " + js.get("name") + " " + js.get("pinyin") + " " + js.get("fullname") + " " + st.split(",\"lat\":")[0] + " " + st.split(",\"lat\":")[1] + " "
                            + id + " 100000 " + 4 + " " + (j + 1));
                }
