...
@Component
@ProtocolCode(3201)
public class SendResponseEquipNoProtocol extends HexConventHandlerAbstract {

    public static final List<String> profile;

    static {
        List<String> protocol = new ArrayList<>(3);
        //key : val (字节长度_别名) math
        protocol.add(0, "4_code");//响应码 3201
    ...
    public String toJsonString(Map<String, Object> data) {
        return "{\"topic\":\"" + data.get("topic") + "\",\"payload\":\"{'data':{'equipid':" + data.get("equipid") + "},'code':" + data.get("code") + "}\"}";
    }
        ...
        if (null != param && null != param.get("field_name") && null != param.get("field_length")) {
            ...
            if (!StringUtils.isEmpty(fieldName) && fieldLength > 0 && null != data.get(fieldName)) {
                ...
                if (fieldLength < 5 && fieldLength > 0) {//默认Int类型处理
                    return HexStringCovert.bytesToHexString(BasicTypeCovert.Int2Bytes_LE_digit(Integer.parseInt(data.get(fieldName).toString()
                    ), fieldLength));
                }
            }
        }
